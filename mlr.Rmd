...
filtered_kc <- expanded_kc %>%
  mutate(
    area_bin = factor(ifelse(!is.na(area_bin), 1, 0)),
  ...
  subset(select = -c(id, date, zipcode, long, lat, yr_renovated)) %>%
...
filtered_kc <- filtered_kc %>%
  subset(select = c(price, sqft_living, bedrooms, bathrooms, grade, area_bin, waterfront, view, yr_built, renovated))
...
transformed <- lm(ystar ~ sqft_living + bedrooms + bathrooms + grade + 
    area_bin + waterfront + view + yr_built + renovated, data = train)
summary(transformed)
```
`long` appears to be insignificant after transforming `bedrooms`

```{r}
transformed_reduced <- lm(ystar ~ sqft_living + bathrooms + grade + 
    area_bin + waterfront + view + yr_built + renovated, data = train)
...
transformed_x_y <- lm(ystar ~ log_sqft_living + bathrooms + grade + 
    area_bin + waterfront + view + yr_built + renovated, data = train)
...
```{r}
boxcox(transformed_x_y)
```
Levene's test
```{r}
ggplot(train, aes(x = waterfront, y = ystar))+
  geom_boxplot()+
  labs(title = 'Dist of Log Price by Waterfront')
```
```{r}
levene.test(train$ystar, train$waterfront)
```

```{r}
levene.test(train$ystar, train$renovated)
```

```{r}
levene.test(train$ystar, train$area_bin)
```
...
```
We have 4 outliers at observations 10903, 10998, 10999, and 19125

```{r}
##observation 1449
train[c('10903', '10998', '10999', '19125'),]
```
...
```{r}
predictors = c('log_sqft_living', 'bathrooms', 'grade', 'view', 'area_bin', 'yr_built', 'waterfront', 'renovated')
generate_partial_regression_plot('ystar', 'log_sqft_living', predictors, train)
```
```{r}
