...
filtered_kc <- expanded_kc %>%
  mutate(
    good_area = factor(ifelse(!is.na(area_bin), 1, 0)), ## using the grades from Niche
...
```{r}
train <- train %>%
  subset(select = c(price, sqft_living, bedrooms, bathrooms, grade, good_area, waterfront, view, yr_built, renovated)) ##training data frame

test <- test %>%
  subset(select = c(price, sqft_living, bedrooms, bathrooms, grade, good_area, waterfront, view, yr_built, renovated)) ##test data frame
```
...
summary(regfull)
```
The predictors in the full regression model all had significant individual t-tests. We will proceed to validate this model.
...
transformed <- lm(log_price ~ sqft_living + bedrooms + bathrooms + grade + 
    good_area + waterfront + view + yr_built + renovated, data = train)
summary(transformed)
```
...
transformed_x_y <- lm(log_price ~ log_sqft_living + bedrooms + bathrooms + grade + view + yr_built + 
    good_area + waterfront + renovated, data = train)
...
```{r}
levene.test(train$log_price, train$good_area)
```
...
For `good_area`, the split used for this analysis is separating the really good areas from the rest. Because of that, the result of the Levene's test is expected.

We will proceed with validating our regression model.
...
```{r}
##dffits
DFFITS <- dffits(transformed_x_y)
length(DFFITS[abs(DFFITS) > 2*sqrt(p/n)])
```
We have no influential pointes based on Cook's distance.
...
```{r}
predictors = c('log_sqft_living', 'bedrooms', 'bathrooms', 'grade', 'view', 'yr_built', 'good_area', 'waterfront', 'renovated')
```
```{r}
